...
void on_ws_message_callback(int web_socket_id, const char *message, int size, void *ptr)
{
    ...
    PeerConnectionNode* node = find_peer_connection_node_by_remote_id(ctx->peer_connection_caches, remote_id);
    if (node != NULL && strcmp(type, "offer") == 0) {
        ...
        node = find_peer_connection_node_by_remote_id(ctx->peer_connection_caches, remote_id);
    } else if (node == NULL) {
